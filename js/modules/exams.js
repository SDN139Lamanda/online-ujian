...
export function saveUjian() {
    ...
    const soalMapel = DB.bankSoal.filter(s => {
        const mapelCocok = s.mapel.toLowerCase() === mapel.toLowerCase();
        if (session.role === 'guru') return mapelCocok && s.guruId === session.user;
        return mapelCocok;
    });
    const jmlSoal = Math.min(parseInt(document.getElementById('ujian-jml-soal').value)||5, soalMapel.length);
    const soalPilih = soalMapel.sort(()=>Math.random()-0.5).slice(0, jmlSoal).map(s => DB.bankSoal.indexOf(s));

    const ujianBaru = {
        id: 'UJ' + Date.now(), guruId: session.user, nama, mapel, kelas, durasi, token,
        status: 'aktif', mulai: document.getElementById('ujian-mulai').value, selesai: document.getElementById('ujian-selesai').value,
        soal: soalPilih,
        opsiAntiCurang: { acakSoal: document.getElementById('opt-acak-soal').checked, acakOpsi: document.getElementById('opt-acak-opsi').checked, fullscreen: document.getElementById('opt-fullscreen').checked, noCopy: document.getElementById('opt-no-copy').checked, tabDetect: document.getElementById('opt-tab-detect').checked, noRightClick: document.getElementById('opt-no-rightclick').checked, satuSesi: document.getElementById('opt-satu-sesi').checked, kamera: document.getElementById('opt-kamera').checked }
    };
    DB.ujian.push(ujianBaru);
...
export function hapusUjian(id) { if (!confirm('Hapus ujian ini?')) return; const idx = DB.ujian.findIndex(u => u.id === id); if (idx !== -1) DB.ujian.splice(idx, 1); showToast('Ujian berhasil dihapus.', 'success'); if (window.renderDataUjian) window.renderDataUjian(); }
export function toggleUjianStatus(id) { const u = DB.ujian.find(u => u.id === id); if (u) u.status = u.status === 'aktif' ? 'selesai' : 'aktif'; showToast(`Status ujian diubah.`, 'info'); if (window.renderDataUjian) window.renderDataUjian(); }

export function bukaTokenModal(ujianId) { currentUjianId = ujianId; openModal('modal-token'); }
export function validateToken() {
    const token = document.getElementById('input-token').value.trim().toUpperCase();
    const ujian = DB.ujian.find(u => u.id === currentUjianId);
    if (!ujian) return;
    if (token !== ujian.token.toUpperCase()) { showToast('Token salah! Periksa kode dari guru Anda.', 'error'); return; }
...
export function filterHasil(ujianId, el) { document.querySelectorAll('.tab-btn').forEach(t => t.classList.remove('active')); el.classList.add('active'); document.getElementById('tabel-hasil').innerHTML = window.renderTabelHasil(ujianId); }

export function exportNilai() {
    const hasilData = DB.hasil.map((h, i) => { const uj = DB.ujian.find(u => u.id === h.ujianId); return { 'No': i + 1, 'Nama Siswa': h.nama, 'Ujian': uj ? uj.nama : h.ujianId, 'Mata Pelajaran': uj ? uj.mapel : '-', 'Nilai': h.nilai, 'Jumlah Benar': h.benar, 'Jumlah Salah': h.salah, 'Soal Kosong': h.kosong, 'Durasi Pengerjaan': h.durasi, 'Pelanggaran': h.pelanggaran, 'Status': h.nilai >= 75 ? 'LULUS' : 'TIDAK LULUS', 'Waktu Kumpul': h.waktu }; });
    if (window.exportToExcel) window.exportToExcel([{ name: 'Rekap Nilai', data: hasilData }], 'Rekap_Nilai_Ujian');
}

export function downloadExcelGuru() { const gurus = Object.entries(DB.users).filter(([, u]) => u.role === 'guru'); const data = gurus.map(([ , u], i) => ({ 'No': i + 1, 'Nama Guru': u.nama, 'NIP': u.nip, 'Mata Pelajaran': u.mapel || '-', 'Username Login': u.nip, 'Password': u.password, 'Status': 'Aktif', 'Keterangan': 'Username login menggunakan NIP' })); if (window.exportToExcel) window.exportToExcel([{ name: 'Data Guru', data }], 'Data_Guru_ExamShield'); showToast('File Excel data guru berhasil diunduh!', 'success'); }
export function downloadExcelSiswa() { const siswas = Object.entries(DB.users).filter(([, u]) => u.role === 'siswa'); const data = siswas.map(([username, u], i) => ({ 'No': i + 1, 'Nama Siswa': u.nama, 'NISN': u.nisn, 'Kelas': u.kelas, 'Username Login': username, 'Password': u.password, 'Status': 'Aktif' })); if (window.exportToExcel) window.exportToExcel([{ name: 'Data Siswa', data }], 'Data_Siswa_ExamShield'); showToast('File Excel data siswa berhasil diunduh!', 'success'); }

export function lihatDetailHasil(id) { const h = DB.hasil.find(r => r.id === id); if (!h) return; document.getElementById('modal-hasil-body').innerHTML = `<div class="grid-2 mb-4"><div><div class="text-sm text-muted">Nama</div><div class="bold">${h.nama}</div></div><div><div class="text-sm text-muted">Nilai</div><div class="bold font-mono" style="font-size:24px;color:${h.nilai>=75?'var(--accent2)':'var(--danger)'};">${h.nilai}</div></div><div><div class="text-sm text-muted">Benar</div><div class="bold text-success">${h.benar} soal</div></div><div><div class="text-sm text-muted">Salah</div><div class="bold text-danger">${h.salah} soal</div></div><div><div class="text-sm text-muted">Durasi</div><div class="bold font-mono">${h.durasi}</div></div><div><div class="text-sm text-muted">Pelanggaran</div><div class="bold ${h.pelanggaran>0?'text-warn':'text-success'}">${h.pelanggaran} kali</div></div></div><div class="alert ${h.nilai>=75?'alert-info':'alert-danger'}"><span>${h.nilai>=75?'✅':'❌'}</span><span>${h.nilai>=75?'Lulus':'Belum memenuhi KKM (75)'} • Dikumpulkan: ${h.waktu}</span></div>`; openModal('modal-hasil'); }
export function printHasil() { window.print(); }
